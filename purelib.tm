...
<\body>
  ...
  <\description>
    ...
    to create a numeric matrix from a pointer, copying the data and
    converting it from the source type on the fly if necessary. The source
  ...
  <\description>
    ...
    The <hlink|<with|font-family|tt|__locals__>|#--locals--> function is
    useful for debugging purposes, as well as to implement dynamic
    environments. It is also used internally to implement the
    <hlink|<with|font-family|tt|reduce>|#reduce> macro, see <hlink|Eval and
    Friends|#eval-and-friends>.

    Note that <hlink|<with|font-family|tt|__locals__>|#--locals--> always
    ...
    Also note that <hlink|<with|font-family|tt|__locals__>|#--locals-->
    will use as keys in the resulting list whatever global symbols are in
    scope at the point of the call. By default, i.e., if no global symbol
    with the same print name as the local is visible at the point of the
    call, a symbol in the default namespace is used, as we've seen above.
    Otherwise the result may be also be a qualified symbol if such a symbol
    has already been declared or defined at the point of the call. For
    instance:
  ...
  <\verbatim>
    ...
    {"Hello, world!",0.333333333333333,4711,#\<less\>pointer 0\<gtr\>}
  </verbatim>
  ...
  <\description>
    ...
    <item*|hmdictp d<label|hmdictp>>check whether <verbatim|x> is a
  ...
  <\description>
    <item*|setp x<label|setp>>

    <item*|bagp x<label|bagp>>

    <item*|hsetp x<label|hsetp>>

    <item*|hbagp x<label|hbagp>>check whether <verbatim|x> is a set or bag of
