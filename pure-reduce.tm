...
<\body>
  ...
  You can check out the latest Reduce version from its svn repository with
  the following command:

  <\verbatim>
    svn co https://reduce-algebra.svn.sourceforge.net/svnroot/reduce-algebra/trunk/
    reduce-algebra
  </verbatim>

  This may take a while; Reduce is a <em|big> package. Once it's finished,
  just drop the resulting reduce-algebra directory into the pure-reduce
  source directory. Run <verbatim|make> and then (if needed) <verbatim|make>
  <verbatim|install> in the pure-reduce directory (<em|not> in the
  reduce-algebra directory!). You should also run <verbatim|make>
  <verbatim|check> which performs a few tests in order to verify that the
  ...
  really minimalistic Reduce frontend. It's small, easy to build and doesn't
  need any support files other than the image file, but it doesn't have any
  of the bells and whistles of the full Reduce version either.

  If you want to install the full Reduce version, there are binary packages
  for various systems available on <hlink|SourceForge|http://sourceforge.net/projects/reduce-algebra/>.
  You can also build Reduce yourself from the sources; please check the build
  instructions in the reduce-algebra folder for details. (Basically, you'll
  need to run <verbatim|./configure> <verbatim|--with-csl> or
  <verbatim|./configure> <verbatim|--with-psl> and then <verbatim|make>
