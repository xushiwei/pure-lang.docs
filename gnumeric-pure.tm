...
<\body>
  ...
  which lets you use <hlink|Pure|http://pure-lang.googlecode.com/>
  functions in Gnumeric, the Gnome spreadsheet. It offers better execution
  speed than the existing Perl and Python plugins, and provides some powerful
  features not found in other Gnumeric scripting plugins, such as
  asynchronous data sources created from Pure streams and OpenGL rendering in
  Gnumeric frame widgets via Pure's OpenGL module.

  <subsection|Introduction<label|introduction>>

  This package provides a <hlink|Gnumeric|http://www.gnumeric.org/>
  extension which gives you access to the
  <hlink|Pure|http://pure-lang.googlecode.com/> programming language in
  ...
  Get the latest source from <hlink|http://pure-lang.googlecode.com/files/gnumeric-pure-0.12.tar.gz|http://pure-lang.googlecode.com/files/gnumeric-pure-0.12.tar.gz>.
  ...
  later since it has improved support for GUI widgets. (Older Gnumeric
  versions probably work as well if you're willing to fiddle with the
  Makefile and/or the sources. See the beginning of the Makefile for related
  information.)

  <with|font-series|bold|Note:> At present, Gnumeric/Pure will <em|not> work
  with the current Gnumeric 1.11 development series which is based on GTK3
  and reportedly has a different plugin interface. (At least nobody has tried
  porting Gnumeric/Pure to it yet.) So you'll have to stick to the stable
  Gnumeric series for the time being.

  As shipped, the Makefile is set up to build Gnumeric/Pure with OpenGL
  support, which requires that you have the OpenGL libraries as well as
  <hlink|GtkGLExt|http://gtkglext.sourceforge.net> (the Gtk OpenGL
  extension) installed. These should be readily available on most systems,
  but you can also disable this feature by invoking <verbatim|make> as
  <verbatim|make> <verbatim|GLDEPS=>.
